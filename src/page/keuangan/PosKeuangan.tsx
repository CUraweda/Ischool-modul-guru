		...
		onSubmit: async (values, { setSubmitting }) => {
			...
			try {
				dataIdxInForm == null
					...
					: await PosPembayaran.update(token, dataList[dataIdxInForm].id, values);
				...
				Swal.fire({
					icon: 'success',
					title: 'Berhasil',
					text: `Berhasil ${dataIdxInForm ? 'memperbarui' : 'menambahkan'} data pos pembayaran`,
				});
				...
				Swal.fire({
					icon: 'error',
					title: 'Oops...',
					text: `Gagal ${dataIdxInForm ? 'memperbarui' : 'menambahkan'} data pos pembayaran`,
				});
			} finally {
				setSubmitting(false);
			}
		...
		<>
			...
			<Modal onClose={() => setDataIdxInForm(null)} id="form-pos-pembayaran">
				<form onSubmit={posPembayaranForm.handleSubmit}>
					<h3 className="text-xl font-bold mb-6">{dataIdxInForm ? 'Edit' : 'Tambah'} Pos Pembayaran</h3>
					<Input
						type="text"
						label="Nama"
						name="name"
					...
					<div className="modal-action">
						...
						<button disabled={posPembayaranForm.isSubmitting} className="btn btn-primary" type="submit">
