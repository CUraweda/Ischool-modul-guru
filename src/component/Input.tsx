	...
	({ className, label, hint, errorMessage, placeholder = 'Ketik disini', ...props }, ref) => {
		return (
			<label className="form-control w-full">
				...
				<textarea
					placeholder={placeholder}
					className={'textarea textarea-bordered w-full min-h-24 max-h-48 ' + className}
					ref={ref}
					{...props}
				/>
				<div className="label">
...
interface SelectProps<T> extends React.InputHTMLAttributes<HTMLSelectElement>, Props {
	options: T[];
	keyValue?: string;
	keyDisplay?: string;
}
				...
				<select className={'text-small select select-bordered w-full ' + className} ref={ref} {...props}>
					...
					{options.map((option, i) => (
						<option
							...
							value={typeof option != 'string' && keyValue ? option[keyValue] : option}
						>
