...
const MapWithTwoRadiusPins: React.FC<FaceDetectionProps> = ({
  onAreas,
  notOnAreas,
}) => {
  ...
  useEffect(() => {
    async function getInitialLocation() {
      try {
        ...
        setError(
          "Failed to access location. Please enable location services and refresh the page."
        ); // Set error state
      }
    }
      ...
      {locationAccessGranted ? (
        <>
          {position2 ? (
            <div className="my-3 w-full flex flex-col justify-center items-center">
              ...
              <span
                className={`${inArea ? "" : "text-red-500 mt-4"} font-semibold`}
              >
                {inArea
                  ? "Mengenali Wajah..."
                  : "Anda Berada Diluar Area, Tidak Dapat Melakukan Presensi"}
              </span>
            </div>
          ) : (
            <div className="my-3 w-full flex flex-col justify-center items-center">
              <span className="font-semibold">Mengambil lokasi...</span>
            </div>
          )}
        </>
      ) : (
        <div className="my-3 w-full flex flex-col justify-center items-center">
          <span className="font-semibold text-red-500">
            Tidak Dapat Mengakses Lokasi. Mohon izinkan akses lokasi untuk
